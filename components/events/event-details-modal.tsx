...
const getRemoteImageUri = (
  source: ReturnType<typeof resolveEventImageSource>,
) => {
  ...
  const normalizedUri = source.uri.trim();
...
export function EventDetailsModal({
  ...
  const imageSource = useMemo(
    () => resolveEventImageSource(event?.image),
    [event?.image],
  );
  ...
  useEffect(() => {
    ...
    const remoteImageUri = getRemoteImageUri(imageSource);
    ...
    void Image.prefetch(remoteImageUri)
      .then((didLoad) => {
  ...
  const isModalLoading = isFetching || isPreparingContent;
  const shouldShowPosterImage = Boolean(imageSource) && !hasImageLoadError;
  const posterImageSource = shouldShowPosterImage ? imageSource ?? undefined : undefined;
              ...
              >
                {shouldShowPosterImage ? (
                  ...
                  <View style={styles.posterFallback}>
                    <Image
                      source={EVENTS_ASSETS.eventPlaceholder}
                      style={styles.posterFallbackIcon}
                      resizeMode="contain"
                    />
                  </View>
                )}
...
const createStyles = (colors: typeof Colors.light) =>
  StyleSheet.create({
    ...
    posterFallback: {
      ...
      alignItems: "center",
      justifyContent: "center",
      backgroundColor: "rgba(8, 21, 37, 0.88)",
    },
    posterFallbackIcon: {
      width: 76,
      height: 76,
